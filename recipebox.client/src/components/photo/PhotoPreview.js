	...
	const { getRootProps, getInputProps, isDragActive, isDragAccept, isDragReject, acceptedFiles, open } = useDropzone({
		...
		onDrop: (acceptedFiles) => {
			!edit
				...
				: setNewFiles(
						acceptedFiles.slice(0, 6 - files.length).map((file) =>
							Object.assign(file, {
								preview: URL.createObjectURL(file)
							})
						)
	...
	return (
		<div className='my-2 text-center'>
			<section className='container'>
				{/* Dealing with preview of new files from a new post and not yet persisted to cloudinary  */}
				{!edit && (
				...
				{!edit && files.length > 0 && <aside style={thumbsContainer}>{thumbs}</aside>}
